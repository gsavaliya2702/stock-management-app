...
const StockIn_1 = __importDefault(require("../models/StockIn"));
const Product_1 = __importDefault(require("../models/Product"));
const router = express_1.default.Router();
// Get all stock-in records
router.get('/', async (req, res) => {
    try {
        const stockIns = await StockIn_1.default.find()
...
router.get('/:id', async (req, res) => {
    try {
        const stockIn = await StockIn_1.default.findById(req.params.id)
            .populate('product_id', 'name unit')
            .populate('supplier_id', 'supplier_name');
        if (!stockIn) {
...
router.post('/', async (req, res) => {
    try {
        ...
        const stockIn = new StockIn_1.default({
        ...
        const populatedStockIn = await StockIn_1.default.findById(newStockIn._id)
            .populate('product_id', 'name unit')
            .populate('supplier_id', 'supplier_name');
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const originalStockIn = await StockIn_1.default.findById(req.params.id);
        if (!originalStockIn) {
            return res.status(404).json({ message: 'Stock-in record not found' });
        }
        ...
        if (req.body.date_received)
            originalStockIn.date_received = req.body.date_received;
        if (req.body.date_expected)
            originalStockIn.date_expected = new Date(req.body.date_expected);
        // If quantity changed, update product stock
        if (quantityDiff !== 0) {
            const product = await Product_1.default.findById(originalStockIn.product_id);
            if (!product) {
                return res.status(404).json({ message: 'Product not found' });
            }
        ...
        // Return populated stock-in record
        const populatedStockIn = await StockIn_1.default.findById(updatedStockIn._id)
            .populate('product_id', 'name unit')
            .populate('supplier_id', 'supplier_name');
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const product = await Product_1.default.findById(stockIn.product_id);
        if (product) {
