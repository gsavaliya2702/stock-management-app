...
async function updateSupplierPerformance(supplierId, deliveryDate, qualityRating) {
    try {
        ...
        // Update supplier
        await Supplier_1.default.findByIdAndUpdate(supplierId, {
            deliverySpeedAvg,
            qualityRatingAvg,
            reliabilityScore,
            performanceScore,
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const existingPurchase = await Purchase_1.default.findById(req.params.id);
        if (!existingPurchase) {
        ...
        if (productId || quantity) {
            const product = await Product_1.default.findById(productId || existingPurchase.productId);
            if (!product) {
                return res.status(404).json({ message: 'Product not found' });
            }
        ...
        // Update the purchase
        const updatedPurchase = await Purchase_1.default.findByIdAndUpdate(req.params.id, updateData, { new: true });
        if (!updatedPurchase) {
            return res.status(404).json({ message: 'Purchase not found' });
        }
        ...
        if (paymentTransactionId && paymentStatus && paymentStatus !== 'pending') {
            try {
                ...
                const existingPaymentHistory = await PaymentHistory_1.default.findOne({
                    transactionId: paymentTransactionId
                });
        ...
        if (supplierId && deliveryDate && qualityRating !== undefined) {
            await updateSupplierPerformance(supplierId, new Date(deliveryDate), qualityRating);
        }
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const purchase = await Purchase_1.default.findById(cleanId);
        if (!purchase) {
        ...
        const supplierPurchases = await Purchase_1.default.find({ supplierId: purchase.supplierId });
        ...
        const stockItem = await StockItem_1.default.findOne({ productId: purchase.productId });
        if (stockItem) {
        ...
        await Purchase_1.default.findByIdAndDelete(cleanId);
